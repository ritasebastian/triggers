...
DECLARE
  ---------------------------------------------------------------------------
  -- Cursors to fetch relevant ACH verification data for the bank account
  ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------
  -- MicroDeposit verification details: amounts, statuses, timestamps, etc.
  ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------
  -- Record variables and output values for staging insert/update
  ---------------------------------------------------------------------------
  ...
  --  v_is_migrated_new ach.t_bav_bank_account_verification.is_migrated%TYPE;
  ...
  ---------------------------------------------------------------------------
  -- INSERT operation: Lookup verification and insert to staging if valid
  ---------------------------------------------------------------------------
-- Open cursor to fetch bank account verification details
begin
...
-- Evaluate verification type and result
...
CLOSE cs_bav_bank_verification;
 -- Assign *_new variables from fetched values
 -- v_result_new := v_result;
 -- v_verification_type_new := v_verification_type;
 -- v_verified_date_new := v_verified_date;
 ...
 -- v_is_migrated_new := v_is_migrated;


-- If verification record was found, fetch related customer and micro-deposit data
IF v_bank_account_verification_id IS NOT NULL THEN
  OPEN cs_customer(v_credit_account_id_new);
  ...
  OPEN cs_md_status(v_bank_account_verification_id);
  ...
  OPEN cs_md_locking(v_bank_account_verification_id);
...
END IF;
  ...
  IF INSERTING AND v_is_migrated != 2 THEN
      -- Final insert to staging table
     /* INSERT INTO ods_stg.STAGE_ACH_EXT_BANK_ACCOUNT (
    ...
    WHEN NOT MATCHED THEN
      INSERT (
        ...
        md2_status,
        md3_status,
        md1_created_timestamp,
        md2_created_timestamp,
        md3_created_timestamp,
        locked,
        ...
        cs_customer_rec.customer_id,
        v_bank_account_id_new,
        v_credit_account_id_new,
        :NEW.routing_number,
        :NEW.account_number,
        CASE :NEW.account_type_id WHEN 1 THEN 'CHK' ELSE 'SAV' END,
        v_result_new,
        v_agent_new,
        v_channel_new,
        v_verification_type_new,
        v_verified_date_new,
        cs_md_rec.mdamount1,
        cs_md_rec.mdamount2,
        cs_md_rec.mdamount3,
        cs_md_status_rec.md1_status,
        cs_md_status_rec.md2_status,
        cs_md_status_rec.md3_status,
        cs_md_ts_rec.md1_created_timestamp,
        cs_md_ts_rec.md2_created_timestamp,
        cs_md_ts_rec.md3_created_timestamp,
        CASE WHEN cs_md_locking_rec.id IS NOT NULL THEN 'Y' ELSE 'N' END,
        0,
        v_Change_timestamp_pst + INTERVAL '0 00:00:00.000011' DAY TO SECOND(6),
        v_agent_new,
        v_created_date_new,
        v_agent_new,
        v_Change_timestamp_pst,
        :NEW.is_primary
      );
  /*ELSIF UPDATING AND :OLD.is_primary != :NEW.is_primary THEN
  IF v_is_migrated != 2 THEN
    -- Update is_primary in staging table for non-migrated records
    ...
    WHERE legacy_customer_id = :NEW.customer_id
      ...
      AND account_type       = CASE :NEW.account_type_id WHEN 1 THEN 'CHK' ELSE 'SAV' END; */
  ...
  IF v_is_migrated != 2  THEN
      ...
      'TRIG_INS_UPD_DEL_T_CUSTOMER_BANK_ACCOUNT', 'T_CUSTOMER_BANK_ACCOUNT', 'customer_bank_account_id:' || TO_CHAR(:NEW.customer_bank_account_id),
      'LEGACY', 'MODERN', 'FAILED',
      'Inside Update and operation v_is_migrated != 2',
      0, 'TRIGGER', SYSTIMESTAMP, '', '', 'EXTERNAL_ACCOUNTS', ''
    );
    -- Update is_primary and account type  in staging table for non-migrated records
    UPDATE ods_stg.STAGE_ACH_EXT_BANK_ACCOUNT
    SET is_primary        = :NEW.is_primary,
  ...
  ELSE
    -- Upsert logic for migrated records
INSERT INTO dbproc.trigger_log (
  trigger_name,
  table_name,
  table_id,
  ...
  'TRIG_INS_UPD_DEL_T_CUSTOMER_BANK_ACCOUNT',
  'T_CUSTOMER_BANK_ACCOUNT',
  'customer_bank_account_id:' || TO_CHAR(:NEW.customer_bank_account_id),
  'LEGACY',
  'MODERN',
  'FAILED',
  'Inside Update and operation v_is_migrated = 2' ||
  ' | ABA: ' || TO_CHAR(:NEW.routing_number) ||
  ' | Account: ' || TO_CHAR(:NEW.account_number) ||
  ' | Credit Card No: ' || TO_CHAR(v_credit_account_id_new) ||
  ' | is_primary_new: ' || TO_CHAR(:NEW.is_primary) ||
  ' | is_primary_old: ' || TO_CHAR(:OLD.is_primary) ||
  ' | account type new: ' || 
    CASE :NEW.account_type_id WHEN 1 THEN 'CHK' ELSE 'SAV' END ||
  ' | account type old: ' ||
    CASE :OLD.account_type_id WHEN 1 THEN 'CHK' ELSE 'SAV' END || 'Flag:' || v_Is_Change_From_MOD ,
  0,
  'TRIGGER',
  SYSTIMESTAMP,
  '',
  ...
  ELSIF DELETING THEN
    ...
    IF v_is_migrated != 2 THEN
      ...
      WHERE legacy_customer_id = :OLD.customer_id
       ...
       -- AND account_type = CASE :OLD.account_type_id WHEN 1 THEN 'CHK' ELSE 'SAV' END;
    ELSE
      OPEN cs_customer(v_credit_account_id);
      FETCH cs_customer INTO cs_customer_rec;
      CLOSE cs_customer;
...
If v_Is_Change_From_MOD='False' THEN
          ...
          USING DUAL
            ON (
                 tgt.legacy_customer_id = cs_customer_rec.customer_id AND
                 tgt.routing_number = :OLD.routing_number AND
                 tgt.account_number = :OLD.account_number 
                )
