...
DECLARE
  ---------------------------------------------------------------------------
  -- Cursors to fetch ACH verification for NEW or OLD bank account event
  ---------------------------------------------------------------------------
  ...
  -- Cursor to get customer_id from credit_account_id
  ...
  ---------------------------------------------------------------------------
  -- Local variables for cursor fetches and staging values
  ---------------------------------------------------------------------------
  ...
  -- Derived/staging values for output to ODS_STG.STAGE_ACH_EXT_BANK_ACCOUNT
...
-- --------------------------------------------------------------------------
begin
  -- Open cursor for verification details for INSERT/UPDATE
  ...
  -- Classify result/type for staging based on verification logic
  ...
  -- Assign other *_new variables for staging
  ...
  -- Fetch customer and micro-deposit details if we have a verification record
  ...
  -- INSERT: add or merge row in staging table for non-migrated
  ...
  -- UPDATE: Update is_primary or account_type in staging (non-migrated)
  ELSIF UPDATING AND (:OLD.is_primary != :NEW.is_primary  OR :OLD.account_type_id != :NEW.account_type_id) THEN
    ...
    ELSE
      -- For migrated, log and upsert the full row if the change was not from MOD
      INSERT INTO dbproc.trigger_log (
        trigger_name, table_name, table_id, source_system, destination_system,
        status, failure_reason, failure_count, created_by, created_timestamp,
        updated_by, updated_timestamp, domain, trigger_input
  ...
  -- DELETE: Mark staging row as INACTIVE, or merge for migrated
  ELSIF DELETING THEN
...
EXCEPTION
  -- Catch-all error log for troubleshooting
  WHEN OTHERS THEN
    INSERT INTO dbproc.trigger_log (
      trigger_name, table_name, table_id, source_system, destination_system,
      status, failure_reason, failure_count, created_by, created_timestamp,
