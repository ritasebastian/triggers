...
DECLARE
    ...
    -- Fetch user record for legacy customer
    ...
    -- Fetch full name from customer master
    ...
    -- Get bank account ID for ABA+account+type
    CURSOR cs_bank IS
      SELECT ID
    ...
    -- Fetch bank name for the routing number
    ...
    -- Fetch active card for the credit account
    CURSOR cs_card IS
      SELECT CARD_ID
      FROM account.t_Card
    ...
    -- Check if customer bank account already exists
    ...
    -- Last 4 digits for logging/audit
...
BEGIN
    ...
    -- Generate reference number for work cases
    SELECT fn_ref_num_seq() INTO v_ref_num FROM DUAL;

    -- Get or create BAV user for this customer
    OPEN cs_bav_user;
    ...
    -- Get or create/update bank account record
    OPEN cs_bank;
    ...
    -- Check if the bank account already exists for this customer
    ...
    IF INSERTING AND :NEW.Single_Use != '1' AND v_is_existing_account = FALSE THEN
        ...
        -- Only process for new bank accounts and operation_type 'ADD'
        IF :NEW.operation_type='ADD' THEN
            OPEN cs_bank;
            FETCH cs_bank INTO bank_rec2;
            CLOSE cs_bank;

            -- If verification needs to be created (StepUp or Agent), create work case and audits
            IF (:NEW.Verification_Type IS NULL AND :NEW.Result='PENDING_ACH_VERIFICATION')
               OR (:NEW.Verification_Type ='AGENT' AND :NEW.Result='ACTIVE') THEN
                ...
                -- Add audit steps for the above work case
                ...
                -- Insert initial verification row (StepUp) for new account
            ...
            -- Always insert final verification, type decided by verification_type/result
            INSERT INTO ACH.T_BAV_BANK_ACCOUNT_VERIFICATION(
                verification_type,
                created_date,
                verified_date,
            ...
            -- If result is ACTIVE, add customer bank account and history
            IF (:NEW.Result='ACTIVE') THEN 
                ...
                -- Add work case for registration with audit trail
                SELECT fnbm_seq.NEXTVAL INTO v_uc_id FROM DUAL;                         
                SELECT fnbm_seq.NEXTVAL  INTO v_uc_audit_id FROM DUAL;
                INSERT INTO t_uc( status_id, type_id, ref_num, open_date, open_agent_id, status_date, status_agent_id, uc_id, credit_account_id, card_id)
                VALUES (1, 325470, v_ref_num, :NEW.CREATED_TIMESTAMP, :NEW.CREATED_BY, :NEW.UPDATED_TIMESTAMP, :NEW.UPDATED_BY, v_uc_id, :NEW.CREDIT_ACCOUNT_ID, v_card_id);
                OPEN cs_bank_name;
                FETCH cs_bank_name INTO cs_bank_name_rec;
                CLOSE cs_bank_name;
                INSERT INTO t_uc_audit (uc_audit_id, agent_id, DATA, audit_date, uc_id) VALUES (v_uc_audit_id, 1,'USE CASE CREATED, STATUS OPEN', :NEW.UPDATED_TIMESTAMP,v_uc_id );
                INSERT INTO t_uc_audit (uc_audit_id, agent_id, DATA, audit_date, uc_id) VALUES (v_uc_audit_id, 1,'Bank Information Updated, '||cs_bank_name_rec.bank_name||', '||:NEW.Routing_Number||', '||:NEW.Account_Number||', '||:NEW.ACCOUNT_TYPE, :NEW.UPDATED_TIMESTAMP,v_uc_id );
                INSERT INTO t_uc_audit (uc_audit_id, agent_id, DATA, audit_date, uc_id) VALUES (v_uc_audit_id, 1,'STATUS CHANGED TO RCLOSED', :NEW.UPDATED_TIMESTAMP,v_uc_id );
            END IF;

            -- Add mapping from modern to legacy bank account ID
            INSERT INTO ODS_STG.LEGACY_MODERN_EXT_ACCT_MAP(
                MODERN_BANK_ACCOUNT_ID,
                LEGACY_BANK_ACCOUNT_ID,
                CREATED_BY,
                CREATED_TIMESTAMP
            ) VALUES(:NEW.MODERN_BANK_ACCOUNT_ID, nvl(bank_rec.id,bank_rec2.id), 'TRIGGER', SYSTIMESTAMP);

            -- If this account is primary, set all others to 0, this to 1, and sync
            IF :NEW.Is_Primary=1 THEN
                ...
                END;
                ods_stg.trig_ctx_pkg.set_from_trig_MBA;                    
                UPDATE account.t_customer_bank_account cba SET cba.is_primary = 0 WHERE cba.customer_id = v_customer_id;
                ods_stg.trig_ctx_pkg.clear_from_trig_MBA;
                ods_stg.trig_ctx_pkg.set_from_trig_MBA; 
                UPDATE account.t_customer_bank_account cba SET cba.is_primary = 1 WHERE cba.customer_bank_account_id = v_generated_id;
                ods_stg.trig_ctx_pkg.clear_from_trig_MBA;
                INSERT INTO account.t_customer_bank_account_sync (customer_bank_account_id, customer_id, change_type)
                VALUES (v_generated_id, v_customer_id, 3);                                   
            END IF;
        END IF; -- END operation_type='ADD'
    END IF; -- END INSERTING

    -- Handle UPDATE events
    IF UPDATING AND :NEW.Single_Use != '1' THEN
        -- If updating from pending verification to pending validation (MD)
        IF :NEW.operation_type='UPDATE' AND :OLD.Result='PENDING_ACH_VERIFICATION' AND :NEW.Result='PENDING_ACH_VALIDATION' 
           and v_is_existing_account = FALSE THEN
            ods_stg.trig_ctx_pkg.set_from_trig_MBA;
            PKG_CUSTOMER_BANK_ACCOUNT.pr_insert_cust_bank_account(:NEW.legacy_customer_id, :NEW.routing_number, :NEW.account_number, CASE WHEN :NEW.account_type='SAV' Then 2 WHEN :NEW.account_type='CHK' Then 1 END, v_generated_id);
            ods_stg.trig_ctx_pkg.clear_from_trig_MBA;                      
            PKG_CUSTOMER_BANK_ACCOUNT.pr_insert_cust_bank_acct_hist(:NEW.legacy_customer_id, 1, :NEW.account_number, :NEW.routing_number, CASE WHEN :NEW.account_type='SAV' Then 2 WHEN :NEW.account_type='CHK' Then 1 END, CASE WHEN :NEW.channel='AGENT' THEN 'CASH' WHEN :NEW.channel='CAS' THEN 'CAS' WHEN :NEW.channel='MOBILE' THEN 'Mobile_App' WHEN :NEW.channel='WEB' Then 'Web' END);
            -- Insert work case and audit for this registration
            SELECT fnbm_seq.NEXTVAL INTO v_uc_id FROM DUAL;
            SELECT fnbm_seq.NEXTVAL INTO v_uc_audit_id FROM DUAL;
            INSERT INTO t_uc(status_id, type_id, ref_num, open_date, open_agent_id, status_date, status_agent_id, uc_id, credit_account_id, card_id)
            VALUES (1, 325470, v_ref_num, :NEW.UPDATED_TIMESTAMP, :NEW.UPDATED_BY, :NEW.UPDATED_TIMESTAMP, :NEW.UPDATED_BY, v_uc_id, :NEW.CREDIT_ACCOUNT_ID, v_card_id);
            OPEN cs_bank_name;
            FETCH cs_bank_name INTO cs_bank_name_rec;
            CLOSE cs_bank_name;
            INSERT INTO t_uc_audit (uc_audit_id, agent_id, DATA, audit_date, uc_id) VALUES (v_uc_audit_id, 1,'USE CASE CREATED, STATUS OPEN', :NEW.UPDATED_TIMESTAMP,v_uc_id );
        ...
        -- If updating from pending validation to ACTIVE (final MD pass)
        IF :NEW.operation_type='UPDATE' AND :OLD.Result='PENDING_ACH_VALIDATION' AND :NEW.Result='ACTIVE' THEN
            -- Find verification record to update
            ...
            Where ba.aba_number = :NEW.routing_number
              ...
              and is_migrated = 2;
            -- If not yet verified, set verified_date/result to pass and audit
        ...
        -- If primary is changed, update primary for all accounts of this customer
        IF (:NEW.operation_type='UPDATE' OR :NEW.operation_type='PRIMARY_UPDATE') 
            ...
            END;
            ods_stg.trig_ctx_pkg.set_from_trig_MBA;
            UPDATE account.t_customer_bank_account cba SET cba.is_primary = 0 WHERE cba.customer_id = :NEW.legacy_customer_id;
            ods_stg.trig_ctx_pkg.clear_from_trig_MBA;
            ods_stg.trig_ctx_pkg.set_from_trig_MBA;
            UPDATE account.t_customer_bank_account cba SET cba.is_primary = 1 WHERE cba.customer_bank_account_id = v_customer_bank_account_id;
            ods_stg.trig_ctx_pkg.clear_from_trig_MBA;
            INSERT INTO account.t_customer_bank_account_sync (customer_bank_account_id, customer_id, change_type)
            VALUES (v_customer_bank_account_id, :NEW.legacy_customer_id, 3);

            PKG_CUSTOMER_BANK_ACCOUNT.pr_insert_cust_bank_acct_hist(:NEW.legacy_customer_id, 3, :NEW.account_number, :NEW.routing_number, CASE WHEN :NEW.account_type='SAV' Then 2 WHEN :NEW.account_type='CHK' Then 1 END, CASE WHEN :NEW.channel='AGENT' THEN 'CASH' WHEN :NEW.channel='CAS' THEN 'CAS' WHEN :NEW.channel='MOBILE' THEN 'Mobile_App' WHEN :NEW.channel='WEB' Then 'Web' END );
        END IF;

        -- Log and handle account/routing number changes
        ...
        -- If account type is changed, update in bank account and history
        IF (:NEW.operation_type='UPDATE' OR :NEW.operation_type='PRIMARY_UPDATE') 
           ...
           AND v_is_existing_account = TRUE AND  cs_existing_account_rec.ACCOUNT_TYPE_id <>  (CASE WHEN :New.ACCOUNT_TYPE='CHK' THEN 1 WHEN :new.ACCOUNT_TYPE='SAV' THEN 2 END ) THEN 
            BEGIN
                SELECT customer_bank_account_id INTO v_customer_bank_account_id
                FROM account.t_customer_bank_account cba
                WHERE cba.customer_id = :NEW.legacy_customer_id
                  AND cba.routing_number=:NEW.routing_number
                  AND cba.account_number=:NEW.ACCOUNT_NUMBER
                  AND cba.account_type_id=CASE WHEN :OLD.ACCOUNT_TYPE='CHK' THEN 1 WHEN :OLD.ACCOUNT_TYPE='SAV' THEN 2 END;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN RETURN;
            END;
            ods_stg.trig_ctx_pkg.set_from_trig_MBA;
            UPDATE account.t_customer_bank_account cba
                SET cba.ACCOUNT_TYPE_ID = CASE WHEN :NEW.ACCOUNT_TYPE='CHK' THEN 1 WHEN :NEW.ACCOUNT_TYPE='SAV' THEN 2 END
                WHERE cba.customer_bank_account_id = v_customer_bank_account_id;  
            ods_stg.trig_ctx_pkg.clear_from_trig_MBA;
            INSERT INTO account.t_customer_bank_account_sync (customer_bank_account_id, customer_id, change_type)
            VALUES (v_customer_bank_account_id, :NEW.legacy_customer_id, 0);
            PKG_CUSTOMER_BANK_ACCOUNT.pr_insert_cust_bank_acct_hist(:NEW.legacy_customer_id, 3, :NEW.account_number, :NEW.routing_number, CASE WHEN :NEW.account_type='SAV' Then 2 WHEN :NEW.account_type='CHK' Then 1 END, CASE WHEN :NEW.channel='AGENT' THEN 'CASH' WHEN :NEW.channel='CAS' THEN 'CAS' WHEN :NEW.channel='MOBILE' THEN 'Mobile_App' WHEN :NEW.channel='WEB' Then 'Web' END );
            -- Add work case/audit for account type change
            SELECT fnbm_seq.NEXTVAL INTO v_uc_id FROM DUAL;
            SELECT fnbm_seq.NEXTVAL INTO v_uc_audit_id FROM DUAL;                                
            INSERT INTO t_uc(status_id, type_id, ref_num, open_date, open_agent_id, status_date, status_agent_id, uc_id, credit_account_id, card_id)
            VALUES (1, 325470, v_ref_num, :NEW.UPDATED_TIMESTAMP, :NEW.UPDATED_BY, :NEW.UPDATED_TIMESTAMP, :NEW.UPDATED_BY, v_uc_id, :NEW.CREDIT_ACCOUNT_ID, v_card_id);
        ...
        -- Handle delete operations (logical, not physical)
        IF :NEW.operation_type='DELETE'  THEN
            ...
            -- Delete bank account if it exists and log in audit
            IF v_customer_bank_account_id is not null THEN  
                ...
                ods_stg.trig_ctx_pkg.clear_from_trig_MBA;
                PKG_CUSTOMER_BANK_ACCOUNT.pr_insert_cust_bank_acct_hist(:NEW.legacy_customer_id, 2, NULL, :NEW.account_number, :NEW.routing_number, CASE WHEN :NEW.account_type='SAV' Then 2 WHEN :NEW.account_type='CHK' Then 1 END, CASE WHEN :NEW.channel='AGENT' THEN 'CASH' WHEN :NEW.channel='CAS' THEN 'CAS' WHEN :NEW.channel='MOBILE' THEN 'Mobile_App' WHEN :NEW.channel='WEB' Then 'Web' END );
                -- Insert work case and audit trail for delete
                SELECT fnbm_seq.NEXTVAL INTO v_uc_id FROM DUAL;
                SELECT fnbm_seq.NEXTVAL INTO v_uc_audit_id FROM DUAL;                          
                INSERT INTO t_uc(status_id, type_id, ref_num, open_date, open_agent_id, status_date, status_agent_id, uc_id, credit_account_id, card_id)
                VALUES (1, 325470, v_ref_num, :NEW.UPDATED_TIMESTAMP, :NEW.UPDATED_BY, :NEW.UPDATED_TIMESTAMP, :NEW.UPDATED_BY, v_uc_id, :NEW.CREDIT_ACCOUNT_ID, v_card_id);
...
EXCEPTION
WHEN OTHERS THEN
    -- Log any error to trigger log table for diagnostics
    INSERT INTO dbproc.trigger_log(trigger_name, table_name, table_id, source_system, destination_system, status, failure_reason, failure_count, created_by, created_timestamp, updated_by, updated_timestamp, domain, trigger_input)
    VALUES (
        'TRIG_INS_STG_MDRN_EXT_BNK_ACCT',
        'STAGE_ACH_MODERN_EXT_BANK_ACCOUNT',
